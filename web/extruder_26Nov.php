...

{
	...
	*/
	...
	k_h = '{$_POST[k_h]}',k_h_bd = '{$_POST[k_h_bd]}',observaciones = '{$_POST[observaciones_extruder]}', 
	k_h = '{$_POST[k_h]}' WHERE id_orden_produccion = '".$ID_ORDENPRODUCCION."'";
				...
				<?php for($a=1;$a<=$nEntradas;$a++) { ?>
					...
					<tr>
                          ...
                          <td align="left"><input class="text2" type="text" name="kg_extruder<?=$codigo[$reg]?>[]"    id="<?=$a?>_extr<?=$id_bd[$reg]?>"  onkeyup="addTextExtruder(<?=$codigo_bd[$reg]?>,<?=$id_bd[$reg]?>,<?=$a?>,<?=$_REQUEST['turno']?>);"  onchange=" sumar('<?=$id[$reg]?>');  sumarExtr(); kh(<?=$_REQUEST['turno']?>);"/></td>
                           -->
                           <td align="left"><input class="text2" type="text" name="kg_extruder<?=$codigo[$reg]?>[]"    id="<?=$a?>_extr_bd<?=$id_bd[$reg]?>"  onkeyup="addTextExtruder_bd(<?=$codigo_bd[$reg]?>,<?=$id_bd[$reg]?>,<?=$a?>,<?=$_REQUEST['turno']?>);"  onchange=" sumar_bd('<?=$id_bd[$reg]?>');  sumarExtr_bd(); kh_bd(<?=$_REQUEST['turno']?>);"/></td>
				  </tr>
...
function probar()
{
    alert("Voy a poner un valor en total extr bd");
	document.getElementById("total_extr_bd").value="22288";
}
