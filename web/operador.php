...
<tr>
    ...
    <option value="6" <? if(($modificar && $dato['area']== 6) || $_REQUEST['buscar'] == 6) echo "selected";?>>R.P.S.</option>
    <option value="7" <? if(($modificar && $dato['area']== 7) || $_REQUEST['buscar'] == 7) echo "selected";?>>S.F.</option>
    ...
    <option value="9" <? if(($modificar && $dato['area']== 9) || $_REQUEST['buscar'] == 9) echo "selected";?>>Calidad</option>
	</select></td>
</tr>
<tr>
...
<tr><td colspan="2" ><table><tr>
	...
	if ($dMaquinas['area'] == 5) echo "R.P.S.";
	if ($dMaquinas['area'] == 6) echo "S.F.";
...
<tr>
  ...
  if($dato['area'] == 6) echo "R.P.S.";
  if($dato['area'] == 7) echo "S.F.";
  if($dato['area'] == 8) echo "Almacen A";
  if($dato['area'] == 9) echo "Calidad";
...
function howMany(form){ 
  ...
  //  var dato = '';
    ...
    //var container2 = document.getElementById('maquina2'); 
        if (numObj > 0) { 
            for(i=1; i<=numObj; i++) { 
                          // html += 'Maquina ' + i + ': <input type="hidden" name="codigos[]" value="'+ i +'"> <select style="width:150" name="id_maquina_'+ i +'" class="contenidos" id="id_maquina_'+ i +'">      <? if($_REQUEST['area'] == 1 || $_REQUEST['area'] == 4) 	$qMaquina = "SELECT * FROM maquina WHERE area = ".$_REQUEST['area']." order by numero"; if($_REQUEST['area'] == 6) 	$qMaquina = "SELECT * FROM maquina WHERE area = 5  order by numero";   if( $_REQUEST['area'] == 7) 	   $qMaquina = "SELECT * FROM maquina WHERE area = 6 ORDER BY numero";  if($_REQUEST['area'] == 3) 	$qMaquina = "SELECT * FROM maquina WHERE area = 2 OR area = 3 ORDER BY area,numero"; $rMaquina = mysql_query($qMaquina);  while ($dMaquina = mysql_fetch_assoc($rMaquina)){?>                 	<option value="<?=$dMaquina['id_maquina']?>"><?=$dMaquina['numero']?> - <?  if($dMaquina['area'] == 1)                  		echo "Camiseta";           	if($dMaquina['area'] == 2)                 		echo "Impresion";                 	if($dMaquina['area'] == 3)                 		echo "Linea de Impr.";                 	if($dMaquina['area'] == 4)    echo "Extruder";                      if($dMaquina['area'] == 5)        echo "RPS";                  if($dMaquina['area'] == 6)            echo "SF"; echo ($dMaquina[tipo_d]==1)?" HD":" BD"?></option> <? } ?>     </select><br>'; 
                                       html += 'Maquina ' + i + ': <input type="hidden" name="codigos[]" value="'+ i +'"> <select style="width:150" name="id_maquina_'+ i +'" class="contenidos" id="id_maquina_'+ i +'">      <? if($_REQUEST['area'] == 1 || $_REQUEST['area'] == 4) 	$qMaquina = "SELECT * FROM maquina WHERE area = ".$_REQUEST['area']." order by numero"; if($_REQUEST['area'] == 6) 	$qMaquina = "SELECT * FROM maquina WHERE area = 6  order by numero";   if( $_REQUEST['area'] == 5) 	   $qMaquina = "SELECT * FROM maquina WHERE area = 5 ORDER BY numero";  if($_REQUEST['area'] == 3) 	$qMaquina = "SELECT * FROM maquina WHERE area = 2 OR area = 3 ORDER BY area,numero"; $rMaquina = mysql_query($qMaquina);  while ($dMaquina = mysql_fetch_assoc($rMaquina)){?>                 	<option value="<?=$dMaquina['id_maquina']?>"><?=$dMaquina['numero']?> - <?  if($dMaquina['area'] == 1)                  		echo "Camiseta";           	if($dMaquina['area'] == 2)                 		echo "Impresion";                 	if($dMaquina['area'] == 3)                 		echo "Linea de Impr.";                 	if($dMaquina['area'] == 4)    echo "Extruder";                      if($dMaquina['area'] == 5)        echo "RPS";                  if($dMaquina['area'] == 6)            echo "SF"; echo ($dMaquina[tipo_d]==1)?" HD":" BD"?></option> <? } ?>     </select><br>'; 
            } 
        } 
//dato = '<? echo $_REQUEST['area'];  ?>';
container.innerHTML = html; 
//container2.innerHTML = dato;
} 
//--> 
</SCRIPT>
  ...
  <tr>
  	<td colspan="9" align="center">
    	<table width="100%" align="center">
          ...
          <td width="16%" align="center" valign="middle"><a href="<?=$_SERVER['HOST']?>?seccion=<?=$_REQUEST['seccion']?>&buscar=9"><img src="images/Calidad.jpg" border="0"></a></td>
          </tr>
          <tr>
          	<td colspan="66">
            	<table width="100%" >
...
<select name='area'  <? if(!$modificar_emp) echo "disabled";  ?> onFocus="Focus(this.id, this.value)" onBlur="Blur(this.id, this.value, 'area', <?=$dListar1['id_operador'];?>)" onChange='cargaContenido(this.id)' class="inputoff" id="d<?=$dListar1['id_operador'];?>" style="text-align:center">
    ...
    <option value="6" <? if($dListar1['area']== 6) echo "selected";?>>R.P.S.</option> 
    <option value="7" <? if($dListar1['area']== 7) echo "selected";?>>S.F.</option> 
    ...
    <option value="9" <? if($dListar1['area']== 9) echo "selected";?>>Calidad</option>     
   	</select>
</td>
 ...
 ?></td>
    <td width="10">
    	...
    	<a href="<?=$_SERVER['PHP_SELF']?>?seccion=<?=$_GET['seccion']?>&rol=<?=$_REQUEST['rol']?>
			...
			&accion=maquina
			&<?=$indice?>=<?=$dListar1['id_operador']?>&area=<? if($dListar1['area'] == 1) echo "4";if($dListar1['area'] == 2) echo "1";if($dListar1['area'] == 3) echo "3";if($dListar1['area'] == 6) echo "5";if($dListar1['area'] == 7) echo "6";?>&nombre=<?=$_REQUEST['nombre']?>">
