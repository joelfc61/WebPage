...
function mostrarExtr(id){
	...
	<? if($concentrado_pegt) 	{ ?>document.getElementById(39).style.display	= "none";<? } ?>
	<? if($extruder_i) 			{ ?>document.getElementById(14).style.display	= "none"; <? } ?>
	<? if($extruder_mh) 		{ ?>document.getElementById(17).style.display	= "none"; <? } ?>
	...
	<? if($concentrado_rpr)		{ ?>document.getElementById(41).style.display		= "none";<? } ?>
...
<form name="form" method="post" id="form" action="admin.php?seccion=<?=$_REQUEST['seccion']?>" >
 ...
 <?  } ?> 
	...
	<td width="48%">
      ...
      <? if($extruder_d){?>
		<p>
        	<label><input type="radio" value="1" name="tipo" onClick="javascript: mostrarExtr(1)" />
            ...
            <p>
              ...
              <? for($meses	= 1; $meses < sizeof($mes); $meses++){ ?>
                 ...
                 <? } ?>
                      ...
                      <? } if($concentrado_pegt){ ?>
					  <p><label><input type="radio" value="39" name="tipo" onClick=" javascript: mostrarExtr('39')" />
            ...
            <span class="style7">HISTORIAL DE MALLAS</span></label></p><br>
   				<p>
                	<div id="17"class="hidden_box" > 
            			<br>
						 <div id="maquinas_impre" >	Maquina:<br> 
                             <?	$qMa	=	"SELECT * FROM maquina WHERE area IN(4) ORDER By area, numero"; 
								$rMa	=	mysql_query($qMa);
            ...
            <br>
			  <p><label><input type="radio" value="42" name="tipo" onClick=" javascript: mostrarExtr('42')" />
                ...
                <? } ?>
                        ...
                        <div id="maquinas" >	Maquinas:<br>
                            <input type="hidden" name="area_maq" value="1">
                            <?	$qMa	=	"SELECT * FROM maquina WHERE area = 4 ORDER By numero"; 
								$rMa	=	mysql_query($qMa);
								$nMa	=	mysql_num_rows($rMa);
							for($a=0;$dMa	=	mysql_fetch_assoc($rMa);$a++){?>
			...
			<b class="xtop"><b class="xb1"></b><b class="xb2"></b><b class="xb3"></b><b class="xb4"></b></b>
				...
				<p align="center">Varios</p><br>
      			<? if($concentrado_pd){ ?>
                    ...
                    <div style="float:left;"><input id="desde" name="desde" class="fecha" type="text" class="fecha" value="<?=date('d-m-Y')?>"   /></div>
...
<? if( isset($_REQUEST['id_reporte'])) {?>
        ...
        <td colspan="9" align="center"><?=$tipo?><input type="button" class="button1" value="Regresar" onClick="javascript: history.go(-1)" id="logo" />&nbsp;&nbsp;<input name="Submit" type="button" class="styleTabla button1" value="Generar Reporte Web" onClick=" paso4();"/></td>
      </tr>
   <? } ?>
    <tr>
      <td>&nbsp;</td>
