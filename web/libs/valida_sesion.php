...
include "conectar.php";
	 ...
	 if($dato3 = mysql_fetch_assoc($resultado3)){	
	  ...
	  if($dato3['maquinaria'] 	== 1){
	   ...
	   if($dato3['empleados'] 	== 1){ 	
	    ...
	    if($dato3['rpsysf'] 		== 1){ 	$areaBolseo		=	$dato3['rpsysf'];		$colspan 	=	$colspan + 1; }
	    ...
	    if($dato3['prenomina_rpsysf'] 	== 1) 		$prenominaRpsysf			=	$dato3['prenomina_rpsysf'];	
	...
	if($tipo_usuario==0){
		...
		//usuarios tipo 1 = supervisor, 2 = vendedor 3 = Administrador
		// agregar a bitacora consec, $_SESSION['user'], $tipo_usuario, date(d-m-y  h:m:s), IP )
?>