...

{
	...
	echo '<script laguaje="javascript">location.href=\'admin.php?seccion=2&accion=nuevo&id_entrada_general='.$id_gen.'&id_orden_produccion='.$id_pro.'&turno='.$turno.'&fecha='.$fecha.'\';</script>';

	} 

	if(!isset($_SESSION['admindolfra']) ){ 

	echo '<script laguaje="javascript">location.href=\'admin_supervisor.php?seccion=2&accion=nuevo&id_entrada_general='.$id_gen.'&id_orden_produccion='.$id_pro.'&turno='.$turno.'&fecha='.$fecha.'\';</script>';
...
//if (!isset($_SESSION['admindolfra'])){
	...
	if($nValidacion  >= 1  && isset($_SESSION['admindolfra'])){
		echo '<script laguaje="javascript">location.href=\'admin.php?seccion=2&accion=registrado\';</script>';
	}

	if($nValidacion  >= 1  && !isset($_SESSION['admindolfra'])){
		echo '<script laguaje="javascript">location.href=\'admin_supervisor.php?seccion=2&accion=registrado\';</script>';
	}
...
<div class="style5" id="div_100" style="display:none; width:300px"><br>Tiempo de fallo: <input type="text" name="horas_fallo" size="2" />:<input type="text" name="minutos_fallo"  size="2" />Hrs.<br />
		...
		<?  
			...
			$a=0;
			// crea 2 arreglos paralelos, $codigo con los numers y $id con los idmaquina
...
<? for($x=1;$x<=4; $x++){?>
 ...
 <td width="190px" align="left" valign="top">
  ...
  <? if($reg<$cant_lic){ 
	...
	<h3 align="left" onclick="muestra_extr('<?=$codigo[$reg]?>');"  class="Tips4" title="Click aqui para abrir o cerrar::"> EXTRUDER <?=$codigo[$reg]?> </h3>
...
-->        <div style="width:90px; float:left; padding-left:8px;" class="style7">Mannto:</div>
  ...
  <?
   //Aqui debe ir lo de  baja densidad;
  ?>
