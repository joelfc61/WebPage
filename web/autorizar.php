...
//habia conexion a dolfra_dolfra, no a la bd  mispruebas
...
//Inicia Guardado datos Extruder
  if(isset($_POST['extruder'])){ // cierra en 438
    ...
    //primero actualiza Orden de produccion
	$qOrdenProduccion	=	"UPDATE orden_produccion  SET total = '{$_POST[total_extruder]}' , desperdicio_tira = '{$_POST[desperdicio_tira]}' , desperdicio_duro = '{$_POST[desperdicio_duro]}',k_h = '{$_POST[k_h]}',total_bd = '{$_POST[total_extruder_bd]}', desperdicio_tira_bd = '{$_POST[desperdicio_tira_bd]}' , desperdicio_duro_bd = '{$_POST[desperdicio_duro_bd]}',  k_h_bd = '{$_POST[k_h_bd]}', observaciones = '{$_POST[observaciones_generales]}' WHERE id_entrada_general = ".$_REQUEST['id_entrada_general']."";
...
//	$ID_ORDENPRODUCCION	=	mysql_insert_id();
  ...
  pDebug("El Tamanio del arreglo de codigos es: ".$nMaquinas);		
  pDebug($_POST['codigos']);
                                 // Para cada maquina en el arreglo
...
"VALUES ('{".$dOrdenID['id_orden_produccion']."}','$id_maquina','$id_operador','$subtotal','$observacion')";
...

		{
			...
			/* Esto sitácticamente puede ir en una línea, pero extrañamente PHP tiene probl con las pilas con asignaciones contiguas */
	...
	if($_REQUEST['menio'] == 0 && !isset($_REQUEST['pendiente']))
	  ...
	  echo '<script language="javascript">location.href=\''.$_SERVER['host'].'?seccion=2&accion=metas&id_entrada_general='.$ID_ORDENPRODUCCION.'&turno='.$_POST['id_turno'].'\';</script>';
 
	else if($_REQUEST['menio'] == 0 && isset($_REQUEST['pendiente']))

	  echo '<script language="javascript">location.href=\''.$_SERVER['host'].'?seccion='.$_REQUEST['seccion'].'&accion=listarPendientes\';</script>';

	else if($_REQUEST['menio'] == 1 && isset($_SESSION['id_admin']))

	  echo '<script language="javascript">location.href=\'admin.php?seccion=30'.$_REQUEST['url'].'\';</script>';   

 

  }  //abre en 82

	/* -- Fin Guardado datos Extruder -- */



	

	/* Comienza  guardado datos Impresión */
	...
	if($_REQUEST['menio'] == 0 && !isset($_REQUEST['pendiente']) && isset($_REQUEST['id_admin']))
    	...
    	echo '<script language="javascript">location.href=\''.$_SERVER['host'].'?seccion=20&accion=metas&id_entrada_general='.$ID_IMPRESION.'&turno='.$_REQUEST['id_turno'].'\';</script>';
	...
	//Fin guardado datos Impresion

	}  

	

// Comienza guardado datos Bolseo 
...
//	$ID_BOLSEO	=	mysql_insert_id();
	...
	echo '<script language="javascript">location.href=\''.$_SERVER['host'].'?seccion=3&accion=metas&id_entrada_general='.$ID_BOLSEO.'&turno='.$_POST['id_turno'].'\';</script>';
...
<hr>
        ...
        <tr>
           <td style="width: 80px"></td><td style="width: 80px"></td>
            ...
            <td><input type="text" style="width: 80px" id="tml" name="tml" value="<?=$dBolseo['millares']?>" class="total"  	<? if(!isset($_SESSION['id_admin'])){ ?>readonly<? } ?> /></td>

            <td><input type="text" style="width: 80px" id="tkg" name="tkg" value="<?=$dBolseo['kilogramos']?>" class="total"	<? if(!isset($_SESSION['id_admin'])){ ?>readonly<? } ?> /></td>

            <td><input type="text" style="width: 80px" id="tdt" name="tdt" value="<?=$dBolseo['dtira']?>" class="total"  		/></td>

            <td><input type="text" style="width: 80px" id="tdtr" name="tdtr" value="<?=$dBolseo['dtroquel']?>" class="total"	/></td>

            <td><input type="text" style="width: 80px" id="tse" name="tse" value="<?=$dBolseo['segundas']?>" class="total" 	 	/></td>
        ...
        <tr>
             ...
             <td colspan="3" align="right" ><strong> Millares por Hora:</strong></td>
	...
	<div id="content">
		...
		<div id="datosgenerales" style="background-color:#FFFFFF;" align="left">
			...
			<p>
                ...
                <CENTER><H1 style="color:#000;font-size:20px;"> PRODUCCION HD </H1></CENTER>
...
<?
            ...
            $sql_lic	=	"SELECT * FROM resumen_maquina_ex INNER JOIN maquina ON resumen_maquina_ex.id_maquina = maquina.id_maquina ".
								" LEFT JOIN operadores ON resumen_maquina_ex.id_operador = operadores.id_operador WHERE (id_orden_produccion=".$dOrdenProduccion['id_orden_produccion'].") and maquina.tipo_d=1  ORDER BY maquina.numero ASC";

            $sql_lic_bd	=	"SELECT * FROM resumen_maquina_ex INNER JOIN maquina ON resumen_maquina_ex.id_maquina = maquina.id_maquina ".
								" LEFT JOIN operadores ON resumen_maquina_ex.id_operador = operadores.id_operador WHERE (id_orden_produccion=".$dOrdenProduccion['id_orden_produccion'].") and maquina.tipo_d=2  ORDER BY maquina.numero ASC";
            
			$res_lic	=	mysql_query($sql_lic) OR die("<p>$qResumenMaquinas</p><p>".mysql_error()."</p>");
			$res_lic_bd	=	mysql_query($sql_lic_bd) OR die("<p>$qResumenMaquinas</p><p>".mysql_error()."</p>");
...
for($i=0;$i<$cant;$i++) //Para la tabla que contiene todas las maquinas
{
  ...
  ?>
   ...
   <? for($x=1;$x<=4; $x++) // Este ciclo crea todos los de una  fila
    {?>

       <td width="206"  align="left" valign="top"><br />

       <? if($reg<$cant_lic){ // si hay un renglon pendiente
		...
		<h3 align="left" style="color: rgb(255, 255, 255);">EXTRUDER <?=$numero[$reg]; echo ' ('.$id_maquina[$reg].')'?><br>
		...
		while ($dOperador = mysql_fetch_assoc($rOperador)) //carga los opeadores en el combo
   ...
   <table width="190" height="59" cellpadding="0" border="0" cellspacing="0" >
	...
	<?php 
            //traerse los detalles de resumen maquina ex
			...
			$nDetalleResumen	=	mysql_num_rows($rDetalleResumen);
             pDebug($qDetalleResumen);
			//Si encontró, detalles, hay O.T. por mostrar 
			...
			for($a = 1;$dDetalleResumen	= 	mysql_fetch_assoc($rDetalleResumen); $a++){	
			  ...
			  <!-- <td > <input type="text" class="text2"  id="<?=$a?>_extr<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]"  onkeydown="addTextExtruder(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$a?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar(<?=$id_maquina[$reg]?>);  sumarExtr();   kh(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" /> </td>  -->

                   <td > <input type="text" class="text2"  id="<?=$a?>_extr<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]"  onkeyup=  "addTextExtruder(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$a?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar(<?=$id_maquina[$reg]?>);  sumarExtrXX(); kh(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" />
                         <input type="hidden" name="id_detalle_resumen_maquina_ex<?=$numero[$reg]?>[]" id="<?=$a?>id_detalle_resumen_maquina_ex<?=$id_maquina[$reg]?>" value="<?=$dDetalleResumen['id_detalle_resumen_maquina_ex']?>">
                   </td>
	...
	<?php for($b=1;$b<=1;$b++) { ?>
				...
				<tr>
                     ...
                     <td><input type="text" class="numeros text2" id="<?=$b?>_extr<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]" onkeyDown="addTextExtruder(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$b?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar(<?=$id_maquina[$reg]?>);  sumarExtrXX(); kh(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" /></td>
<!--				old	 <td><input type="text" class="numeros text2" id="<?=$b?>_extr<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]" onKeyDown="addTextExtruder(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$b?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar(<?=$id_maquina[$reg]?>);  sumarExtr(); kh(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" />  -->

                      <input type="hidden" name="id_detalle_resumen_maquina_ex<?=$numero[$reg]?>[]" id="<?=$a?>id_detalle_resumen_maquina_ex<?=$id_maquina[$reg]?>" value="0"> 
                     </td>
				...
				<!--  <td><input style="width:100px" type="text" name="subtotal_extruder<?=$numero[$reg]?>" id="subtotal_<?=$id_maquina[$reg]?>_extr_Totalh" onChange="sumarExtr();"  readonly="readonly" value="<?=$subtotales[$reg]?>" /></td> -->
                      <td><input style="width:100px" type="text" name="subtotal_extruder<?=$numero[$reg]?>" id="subtotal_<?=$id_maquina[$reg]?>_extr_Totalh" onChange="sumarExtr();"  readonly="readonly" value="<?=$subtotales[$reg]?>" /></td>
					</tr>
   ...
   <div id="datosgenerales" style="background-color:#FFFFFF;">
        ...
        <table align="center" border="0">
					...
					<tr>
						...
						<td><input type="text" id="total_extr" name="total_extruder" value="<?=$dOrdenProduccion['total']?>" class="total" readonly  /></td>
			            ...
			            <td><input type="text" size="20" name="k_h" id="k_h" class="total" value="<?=$dOrdenProduccion['k_h']?>" readonly/></td>
       ...
       <h1> <HR>
       <!--   Aqui comienza Reporte de produccion BD  -->

    <p align="center">          
    <h1 style="color:#f00;">  PRODUCCION BD </H1>
 ...
 <?
    ...
    $res_lic	=	mysql_query($sql_lic) OR die("<p>Error:</p><p>".mysql_error()."</p>");
    ...
    for($i=0;$i<$cant;$i++) { ?>
       ...
       <? 
		...
		<h3 align="left" style="color: rgb(255, 255, 255);background-color:#FE642E;">EXTRUDER <?=$numero[$reg];echo ' ('.$id_maquina[$reg].')';?><br>
      ...
      for($a = 1;$dDetalleResumen = mysql_fetch_assoc($rDetalleResumen); $a++){	
		?>
	          <tr>
	           <td height="19px" ><input type="text"  class="text" id="ot_extruder_bd<?=$id_maquina[$reg]?>_<?=$a?>" name="ot_extruder<?=$numero[$reg]?>[]" onFocus="this.select()"  value="<?=$dDetalleResumen['orden_trabajo']?>" /></td>
	           <td ><input type="text" class="text2" id="<?=$a?>_extr_bd<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]"  onkeyup="addTextExtruder_bd2(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$a?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar_bd(<?=$id_maquina[$reg]?>);  sumarExtr_bd();  kh_bd(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" />
	     ...
	     <?php for($b=1;$b<=1;$b++) { ?>
          	<tr>
	          <td ><input type="text"  class="numeros text" id="ot_extruder<?=$id_maquina[$reg]?>_<?=$b?>" name="ot_extruder<?=$numero[$reg]?>[]" onFocus="this.select()" value="<?=$dDetalleResumen['orden_trabajo']?>" /></td>
               <td><input type="text" class="numeros text2" id="<?=$b?>_extr_bd<?=$id_maquina[$reg]?>" name="kg_extruder<?=$numero[$reg]?>[]" onKeyDown="addTextExtruder_bd2(<?=$numero[$reg]?>,<?=$id_maquina[$reg]?>,<?=$b?>,<?=$dEntradaGeneral['turno']?>);" onchange="sumar_bd(<?=$id_maquina[$reg]?>);  sumarExtr_bd();  kh(<?=$dEntradaGeneral['turno']?>);" value="<?=$dDetalleResumen['kilogramos']?>" />
		...
		<tr>
 		 <td height="20" style="font-size:9px"><strong>Subtotales</strong></td>
        	 <td><input style="width:100px" type="text" name="subtotal_extruder<?=$numero[$reg]?>" id="subtotal_<?=$id_maquina[$reg]?>_extr_Total_bd" onChange="sumarExtr_bd();"  readonly="readonly" value="<?=$subtotales[$reg]?>" /></td>
  	       </tr>
   </table>
  ...
  <? }?>
   ...
   <div id="datosgenerales" style="background-color:#FFFFFF;">
  	 ...
  	 <tr>
 	  <td><strong>Totales</strong></td>
	  <td><input type="text" id="total_extr_bd" name="total_extruder_bd" value="<?=$dOrdenProduccion['total_bd']?>" class="total" 						
          readonly /></td>
	  <td><input type="text" id="desperdicio_tira_bd" name="desperdicio_tira_bd" value="<?=$dOrdenProduccion['desperdicio_tira_bd'];?>" class="total"  /></td>
 	  <td><input type="text" id="desperdicio_duro_bd" name="desperdicio_duro_bd" value="<?=$dOrdenProduccion['desperdicio_duro_bd'];?>" class="total"  /></td>

          <td><input type="text" size="20" name="k_h_bd" id="k_h_bd" class="total" value="<?=$dOrdenProduccion['k_h_bd']?>" readonly/></td>
         </tr>
         <tr>
          <td valign="top">Observaciones:</td>
          <td colspan="4" valign="top"><textarea cols="50" rows="6" name="observaciones_generales"><?=$dOrdenProduccion['observaciones'];?></textarea></td>
...
<!--   Aqui termina Reporte de produccion BD  -->
<HR></h1>

        <div id="datosgenerales" style="background-color:#FFFFFF;" align="left">

        <p align="center" class="titulos_reportes"><br />TIEMPOS MUERTOS HD</p>         

           

		<?  

		 	$sql_lic= "SELECT * FROM tiempos_muertos INNER JOIN maquina ON tiempos_muertos.id_maquina = maquina.id_maquina  WHERE id_produccion	= '".$dOrdenProduccion['id_orden_produccion']."' AND tipo= 1 and tipo_d=1 ORDER BY maquina.numero ASC ";
...
-->        <div style="width:90px; float:left; padding-left:8px;" class="style7">Mtto:</div>
...
<CENTER>
<HR>
<h1>TIEMPOS MUERTOS BD </h1>

		<?  

		 	$sql_lic= "SELECT * FROM tiempos_muertos INNER JOIN maquina ON tiempos_muertos.id_maquina = maquina.id_maquina  WHERE id_produccion	= '".$dOrdenProduccion['id_orden_produccion']."' AND tipo= 1 and tipo_d=2 ORDER BY maquina.numero ASC ";
...
<td width="190px" align="left" valign="top">
  ...
  <? if($reg<$cant_lic){ ?>		
		...
		<h3 align="left" style="color: rgb(255, 255, 255); background-color:#FE642E;"  class="Tips4" title="Click aqui para abrir o cerrar:: &nbsp;" onClick="muestra_extr('<?=$codigo[$reg]?>');">EXTRUDER <?=$codigo[$reg]?> </h3>
...
-->        <div style="width:90px; float:left; padding-left:8px;" class="style7">Matto:</div>
...
<div id="container">
	...
	<div id="content">
      ...
      <p class="titulos_reportes">Impresi&oacute;n - Nave 2</p>
                 ...
                 <? }
                  ...
                  <div id="limprtexto<?=$id_maquina2[$reg2]?>"></div>	
					...
					<tr>
						...
						<td style="font-size:9px"><strong>Subtotal HD</strong><br /><strong>Subtotal BD</strong></td>
		...
		<h3 ><a style="color: rgb(255, 255, 255);" href="#extruder">REPORTES EXTRUDER</a></h3>
            ...
            <div style="background-color:#FFFFFF;" class="navcontainer">
              ...
              <table>
				...
				$qGenerales		.=	"DATEDIFF(CURRENT_DATE(),fecha) <= 31"; //Que no tenga más de 3 días de la creación
...
<tr>
				...
				$qGenerales2		.=	"DATEDIFF(CURRENT_DATE(),fecha) <= 31"; //Que no tenga más de 3 días de la creación
...
<? } if(($_SESSION['area3'] == 1 && isset($_GET['bolseo'])) || isset($_SESSION['id_admin'])){ ?>
      ...
      <tr>
				...
				$qBolseo		.=	"DATEDIFF(CURRENT_DATE(),fecha) <= 31"; //Que no tenga más de 3 días de la creación
...
<tr>
	...
	<td align="center">
		...
		<div class="tablaCentrada">
		    ...
		    Se ha actualizado el registro  en el sistema.<br>
