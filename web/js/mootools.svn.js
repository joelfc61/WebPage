	...
	*/
	...
	makeEditable:function(options){
		this.options = {
			...
			contentNameVar: 'contentName',
			actionNameVar: 'action',
			actionContent: 'edit',
			idValue:'1',
			event:'click',
			contentField:'content',
			onComplete: function(item){
				if($type(item.options) != 'object'){
					return false;
				}
				var page = item.options.page;
				if(item.ajaxOptions.method.toLowerCase() =='post'){
 					var things	=	item.options.idName+"="+item.options.idValue+"&";
					things		+=	item.options.nameFieldVar+'='+item.options.contentField+'&';
					things		+=	item.options.actionNameVar+'='+item.options.actionContent+'&';
					things		+=	item.options.contentNameVar+'='+escape(item.getHTML());
					item.ajaxOptions.data = things;
				}else{
					if(page.indexOf("?") && page.indexOf("?") != -1){
						if(page.indexOf("&") !=-1 && page.indexOf("&")==page.length-1){
							page+=idName+"="+idValue+"&"+contentField+"="+escape(item.getHTML());
						}else{
							page+="&"+idName+"="+idValue+"&"+contentField+"="+escape(item.getHTML());
						}
					}else{
					...
					item.options.page = page;
				}
 
				var ajx = new Ajax(item.options.page,item.ajaxOptions);
				ajx.request();
 
			},
			//The function to run when the onBlur event is completed;
			onBlur: function(item){
				var parent = item.parentNode;
				parent.changing= false;
				//Set the changing variable to false
				parent.setHTML(item.value);
				//Set the HTML
				parent.options.onComplete(parent);
				//Run the onComplete function;
			},//Changes the item back to its original state, just with modified content;
 
			onKeyPress: function(event,type,item){
				var event = ($defined(event)) ? event : window.event;
				var type = ($defined(type)) ? type : 'text';
				var item = ($type(item)=='string') ? $(item) : item;
				if(!$defined(item)){
					return false;
					//Hacking attempt;
				}
 				if(event.ctrlKey && (event.keyCode==13 || event.keyCode==10 ) &&  type=='text'){item.blur();}
 				if(event.keyCode=='13' && type=='input'){ item.blur(); }
 			},//End onKeyPress function;
 			container:null
			//the button to work with
 		}
		//End options;
		if($defined(options)){
			this.ajaxOptions = $merge(this.options.ajaxOptions,options.ajaxOptions);
		}else{
			this.ajaxOptions = this.options.ajaxOptions;
		}
 
		if(this.id){			
			if($defined(this.changing)){
				this.changing=false;
				//if this.changing is not defined, define it;
			}
 
			for(x in options){
 				if($type(options[x]) == 'function'){
					if(x.match(/^on[A-Z]{1}[a-z]+/)){
						this.options[x] = options[x];
					}
 				}else{
					this.options[x] = options[x];
				}
			}//End for(x in options)
			//If this has an ID, set the options;
			var thing = ($defined(this.options.container)) ? this.options.container : this;
 
			thing = ($type(thing))? $(thing) : thing;
 
			var passVariable = ($defined(this.options.container)) ? this.id : this;
 
			thing['on'+this.options.event] = function(item){
				item = ($type(item)=='string') ? $(item) : item;
				if($defined(item.id) && !item.changing){
					var HTML = item.getHTML();
					//Get the Current HTML;
					var sizes = item.getSize().scrollSize;
					//Get the current sizes;
 
					if(sizes.y + 2 <=40){
						item.setHTML("<input type=\"text\" onblur=\"this.parentNode.options.onBlur(this)\" onkeypress=\"this.parentNode.options.onKeyPress(event,'input',this)\" id=\""+item.id+"_field\" value=\""+HTML+"\" style=\"width:"+sizes.x+"px;height:"+sizes.y+"px;\" />");
					}else{
						item.setHTML("<textarea id=\""+item.id+"_field\" onkeypress=\"this.parentNode.options.onKeyPress(event,'text',this)\" onblur=\"this.parentNode.options.onBlur(this)\" style=\"width:"+sizes.x+"px;height:"+sizes.y+"px\">"+HTML+"</textarea>");
					}
					item.changing=true;
				}else{
					return false;
			...
			//end this['on'+..] = function(item) line;
		}
...
Element.extend({
	...
	*/
	...
	send: function(options){
		return new Ajax(this.getProperty('action'), $merge({data: this.toQueryString()}, options, {method: 'post',evalScripts: true})).request();
	}
