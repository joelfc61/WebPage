...
{
	//Extruder
  	$qActualizaExtruder = "UPDATE orden_produccion SET desperdicio_tira = '".$_REQUEST['desperdicio_tira']."' , desperdicio_duro = '".$_REQUEST['desperdicio_duro']. "',  desperdicio_tira_bd = '".$_REQUEST['desperdicio_tira_bd']."' , desperdicio_duro_bd = '".$_REQUEST['desperdicio_duro_bd']."' WHERE id_entrada_general = '".$_REQUEST['id_extruder']."' ";
	$rActualizaExtruder = mysql_query($qActualizaExtruder);
	 $qOrden = "UPDATE entrada_general SET actualizado = '1', repesada=1,  fecha_repesada='".date('Y-m-d')."' WHERE id_entrada_general =  '".$_REQUEST['id_extruder']."' ";
 	 pDebug($qActualizaExtruder);
 	$rOrden = mysql_query($qOrden);
     pDebug($qOrden);    
	//EN CASO DE QUE EL ADMINISTRADO DE UNA ENTRADA DE REPESADA.
	if(isset($_REQUEST['id_supervisor'])){
		$qRepesada	=	"INSERT INTO repesadas (id_admin, id_supervisor, turno, id_entrada_general, desperdicio1, desperdicio2,desperdicio3,desperdicio4, diferencia1, diferencia2,diferencia3,diferencia4) VALUES('0','{$_REQUEST[id_supervisor]}','{$_REQUEST[turno]}','{$_REQUEST[id_extruder]}', '{$_POST[desperdicio_tira]}','{$_POST[desperdicio_duro]}' ,'{$_POST[desperdicio_tira_bd]}','{$_POST[desperdicio_duro_bd]}' ,'{$_POST[diferencia_tira]}','{$_POST[diferencia_duro]}','{$_POST[diferencia_tira_bd]}','{$_POST[diferencia_duro_bd]}')";
 	}
	if(!isset($_REQUEST['id_supervisor'])){
	 $qRepesada	=	"INSERT INTO repesadas (id_admin, id_supervisor, turno, id_entrada_general, desperdicio1, desperdicio2, desperdicio3, desperdicio4, diferencia1, diferencia2,diferencia3,diferencia4) VALUES('{$_REQUEST[id_admin]}','{$_REQUEST[id_supervisor]}','{$_REQUEST[turno]}','{$_REQUEST[id_extruder]}','{$_POST[desperdicio_tira]}','{$_POST[desperdicio_duro]}' ,'{$_POST[desperdicio_tira_bd]}','{$_POST[desperdicio_duro_bd]}' ,'{$_POST[diferencia_tira]}','{$_POST[diferencia_duro]}','{$_POST[diferencia_tira_bd]}','{$_POST[diferencia_duro_bd]}')";
	}
...
// ACTUALIZA LAS MODIFICACIONES extruder
if(isset($_POST['guardar']) && ($_REQUEST['actualiza'] == 1 )  && (isset($_REQUEST['id_extruder']) && isset($_REQUEST['id_bolseo']) && isset($_REQUEST['id_impresion']) ))
{
    ...
    pDebug($qOrden);
	if($nBuscarE < 1)
  	 $qRepesada	=	"INSERT INTO repesadas  (id_admin, id_entrada_general, desperdicio1, desperdicio2, desperdicio3,desperdicio4,diferencia1 ,diferencia2,diferencia3,diferencia4) VALUES('{$_POST[id_admin]}', '{$_REQUEST[id_extruder]}' ,'{$_POST[desperdicio_tira]}','{$_POST[desperdicio_duro]}','{$_POST[desperdicio_tira_bd]}','{$_POST[desperdicio_duro_bd]}','{$_POST[diferencia_tira]}', '{$_POST[diferencia_duro]}','{$_POST[diferencia_tira_bd]}', '{$_POST[diferencia_duro_bd]}') ";
	if($nBuscarE > 0)
	 $qRepesada	=	"UPDATE repesadas SET id_admin = '{$_POST[id_admin]}',  desperdicio1 = '{$_POST[desperdicio_tira]}' , desperdicio2 = '{$_POST[desperdicio_duro]}', desperdicio3 = '{$_POST[desperdicio_tira_bd]}' , desperdicio4 = '{$_POST[desperdicio_duro_bd]}',  diferencia1 = '{$_POST[diferencia_tira]}', diferencia2 = '{$_POST[diferencia_duro]}', diferencia3 = '{$_POST[diferencia_tira_bd]}', diferencia4 = '{$_POST[diferencia_duro_bd]}'  WHERE id_entrada_general = '{$_REQUEST[id_extruder]}' ";
	$rRepesada	=	mysql_query($qRepesada);
	pDebug($qRepesada);
//actualiza modificaciones impresion
...
//actualiza modificaciones bolseo
...
if(!empty($_GET['accion']))
{
	...
	{	
		...
		$qOrdenProduccion	=	"SELECT  total, desperdicio_tira, desperdicio_duro,desperdicio_tira_bd, desperdicio_duro_bd FROM orden_produccion WHERE (id_entrada_general = {$dExtruder[id_entrada_general]})";
...
<tbody>
        			...
        			<tr>
        			  <td colspan="3"><h3 align="center">EXTRUDER             H.D.</h3></td>
					  <td colspan="2"><h3 align="center" style="background-color:#FF8533"> B.D.</h3></td>
					...
					<tr>
						...
						<td><input type="text" id="desperdicio_tira_s" name="desperdicio_tira_s" value="<? if($modificar){ echo  $dRepedasaExtr['desperdicio1'];} if($repesar){ echo  $dOrdenProduccion['desperdicio_tira']; }  ?>" class="total"  readonly="readonly"/></td>
						<td><input type="text" id="desperdicio_duro_s" name="desperdicio_duro_s" value="<? if($modificar){ echo  $dRepedasaExtr['desperdicio2'];} if($repesar){ echo  $dOrdenProduccion['desperdicio_duro']; }   ?>" class="total" readonly="readonly"/></td>
						<td><input type="text" id="desperdicio_tira_bd_s" name="desperdicio_tira_bd_s" value="<? if($modificar){ echo  $dRepedasaExtr['desperdicio3'];} if($repesar){ echo  $dOrdenProduccion['desperdicio_tira_bd']; }  ?>" class="total" readonly="readonly"  /></td>
						<td><input type="text" id="desperdicio_duro_bd_s" name="desperdicio_duro_bd_s" value="<? if($modificar){ echo  $dRepedasaExtr['desperdicio4'];} if($repesar){ echo  $dOrdenProduccion['desperdicio_duro_bd']; }  ?>" class="total" readonly="readonly"   /></td>
					</tr>
					<tr>
						...
						<td><input type="text" id="desperdicio_tira_bd" name="desperdicio_tira_bd" value="<? if($modificar){ echo	 $dOrdenProduccion['desperdicio_tira_bd']; } ?>" onChange="javascript: diferencia('desperdicio_tira_bd_s',this.id,'diferencia_tira_bd');" class="total"  /></td>
						<td><input type="text" id="desperdicio_duro_bd" name="desperdicio_duro_bd" value="<? if($modificar){ echo	 $dOrdenProduccion['desperdicio_duro_bd']; } ?>" onChange="javascript: diferencia('desperdicio_duro_bd_s',this.id,'diferencia_duro_bd');" class="total"  /></td>
					...
					<tr>
						...
						<td><input type="text" id="diferencia_tira_bd" name="diferencia_tira_bd" value="<? if($modificar){ echo $dRepedasaExtr['diferencia3'];} ?>" class="total" readonly="readonly"  /></td>
						<td><input type="text" id="diferencia_duro_bd" name="diferencia_duro_bd" value="<? if($modificar){ echo $dRepedasaExtr['diferencia4'];} ?>" class="total" readonly="readonly"  /></td>						
        ...
        <table width="650" align="center" >
          <tbody>
            <tr>
              <td colspan="3" ><h3 align="center">IMPRESION H.D.</h3></td>
              <td colspan="2"><h3 align="center" style="background-color:#ff8533"> B.D.</h3></td>
            </tr>
            
            <tr>
              <td width="180"></td>           
              <td width="150">D.Tira H.D.</td>
              <td width="50"></td>
              <td width="200" >D.Tira B.D.</td>
                <td>&nbsp;</td>
            </tr>
            <tr>
              ...
              <td><input type="text" id="total_desperdicio_hd_s" name="total_desperdicio_hd_s" value="<?  if($modificar){ echo  $dRepedasaImp['desperdicio1']; } if($repesar){ echo $dImpresion['desperdicio_hd']; }?>" class="total" readonly="readonly" /></td>
              <td>&nbsp;</td>
              <td><input type="text" id="total_desperdicio_bd_s" name="total_desperdicio_bd_s" value="<?  if($modificar){ echo  $dRepedasaImp['desperdicio2']; } if($repesar){ echo $dImpresion['desperdicio_bd']; }?>" class="total" readonly="readonly"  /></td>
              <td >&nbsp;</td>
            </tr>
            <tr>
              <td><strong>Repesada</strong></td>
            ...
            <tr>
              ...
              <td><input type="text" id="diferencia_hd" name="diferencia_hd" value="<? if($modificar){ echo $dRepedasaImp['diferencia1'];} ?>" class="total" readonly="readonly"  /></td>
                <td>&nbsp;</td>
              <td><input type="text" id="diferencia_bd" name="diferencia_bd" value="<? if($modificar){ echo $dRepedasaImp['diferencia2'];} ?>" class="total" readonly="readonly"  /></td>
              <td >&nbsp;</td>
            </tr>
            <tr>
        ...
        <table width="650" align="center" border="0">
          ...
          <thead>
            <tr>
              <td colspan="4"><h3 align="center">BOLSEO H.D.</h3></td>
              <td colspan="2"><h3 align="center" style="background-color:#ff8533">RPSySF HD</h3></td>
              <td colspan="2"><h3 align="center" style="background-color:#ff8533">RPSySF BD</h3></td>              
            </tr>
            <tr>
              <td>&nbsp;</td>            
              <td><b>D.Tira</b></td>
          ...
          <tbody>
            <tr>
              <td><strong>Totales</strong></td>         
              <td><input type="text" id="tdt_s"  name="tdt_s"  value="<? if($modificar){ echo $dRepedasaBol['desperdicio1'];} if($repesar){ echo $dActualizaBolseo['dtira']; 	}  ?>"   class="total" readonly="readonly"/></td>
              <td><input type="text" id="tdtr_s" name="tdtr_s" value="<? if($modificar){ echo $dRepedasaBol['desperdicio2'];} if($repesar){ echo $dActualizaBolseo['dtroquel'];	} ?>"   class="total" readonly="readonly" /></td>
              <td><input type="text" id="tse_s"  name="tse_s"  value="<? if($modificar){ echo $dRepedasaBol['desperdicio3'];} if($repesar){ echo $dActualizaBolseo['segundas'];	} ?>"   class="total" readonly="readonly"/></td>

              <td><input type="text" id="hd_t_rps"  name="hd_t_rps"  value="."   class="total" readonly="readonly"/></td>
              <td><input type="text" id="hd_t_sf"   name="hd_t_sf"  value="."   class="total" readonly="readonly"/></td>
              <td><input type="text" id="bd_t_rps"  name="bd_t_rps"  value="."   class="total" readonly="readonly"/></td>
              <td><input type="text" id="bd_t_sf"   name="bd_t_sf"  value="."   class="total" readonly="readonly"/></td>              
            </tr>
            <tr>
              <td><strong>Repesada</strong></td>  
              ...
              <td><input type="text" id="hd_r_rps"  name="hd_r_rps"  value="."   class="total" onchange=""  /></td>
              <td><input type="text" id="hd_r_sf"  name="hd_r_sf"  value="."   class="total" onchange=""  /></td>
              <td><input type="text" id="bd_r_rps"  name="bd_r_rps"  value="."   class="total" onchange=""  /></td>
              <td><input type="text" id="bd_r_sf"  name="bd_r_sf"  value="."   class="total" onchange="" /></td>              
            ...
            <tr>
              ...
              <td><input type="text" id="hd_d_rps"  name="hd_d_rps"  value="<? //if($modificar){ echo $dRepedasaBol['diferencia1'];} ?>"   class="total" readonly="readonly"  /></td>
              <td><input type="text" id="hd_d_sf" name="hd_d_sf" value="<? //if($modificar){ echo $dRepedasaBol['diferencia2'];} ?>"   class="total" readonly="readonly" /></td>
              <td><input type="text" id="bd_d_rps"  name="bd_d_rps"  value="<? //if($modificar){ echo $dRepedasaBol['diferencia3'];} ?>"   class="total" readonly="readonly"  /></td>
              <td><input type="text" id="bd_d_sf"  name="bd_d_sf"  value="<? //if($modificar){ echo $dRepedasaBol['diferencia3'];} ?>"   class="total" readonly="readonly"  /></td>
 
            </tr>
            <tr>
              <input type="hidden" name="extruder" id="extruder" value="extruder" />
              <td width="214" align="center" colspan="6"><input type="submit" name="guardar" id="Guardar" value="Guardar"  />
              <input name="id_extruder" type="hidden" id="id_extruder" value="<?=$dExtruder['id_entrada_general']; ?>" />
    		  ...
    		  <input type="hidden" name="actualiza" value="<? if($repesada) echo '0'; if($modificar) echo '1';?>" /></td>
          	</tr>
