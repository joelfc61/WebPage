...

{
	...
	*/
	...
	$qOrdenProduccion	=	"UPDATE  orden_produccion SET id_entrada_general = '$ID_GENERAL', total = '{$_POST[total_extruder]}', desperdicio_tira = '{$_POST[desperdicio_tira_extruder]}', desperdicio_duro = '{$_POST[desperdicio_duro_extruder]}', observaciones = '{$_POST[observaciones_extruder]}', k_h = '{$_POST[k_h]}' WHERE id_orden_produccion = '".$ID_ORDENPRODUCCION."'";
...

	{
		...
		$qResumenMaquinaEx	=	"INSERT INTO resumen_maquina_ex (id_orden_produccion, id_maquina, id_operador, subtotal, observacion) ".
								...
								"VALUES ('$ID_ORDENPRODUCCION','$id_maquina','$id_operador','$subtotal','$observacion')";
   ...
   <? for($x=1;$x<=4; $x++){?>
    ...
    <td width="220" align="left" valign="top"><br />
     ...
     <? if($reg<$cant_lic){ ?>		
		...
		<h3 align="left" style="color: rgb(255, 255, 255);">Extruder <?=$codigo[$reg]?><br />
			  ...
			  <td width="126" ><strong>Kilogramos</strong></td>
				...
				<?php } ?>
					...
					<tr>
						...
						<td><input style="width:100px" type="text" name="subtotal_extruder<?=$codigo[$reg]?>" id="subtotal_<?=$id[$reg]?>_extr_Total" onChange="sumarExtr();"  readonly="readonly" /></td>
     ...
     <? if($reg<$cant_lic_bd){ ?>		
		...
		<?
			...
			$dAsignacionextr = mysql_fetch_assoc($rOperadorextr);  ?>
	 		...
	 		<input type="hidden"  name="id_operador<?=$codigo_bd[$reg]?>" value="<?=$dAsignacionextr['id_operador'] ?>" />

            <input type="hidden" name="id_maquina<?=$codigo_bd[$reg]?>" value="<?=$id_bd[$reg]?>" />

            <input type="hidden" name="codigos[]" value="<?=$codigo_bd[$reg]?>" />

		<h3 align="left"   style="background-color:#FE642E;color: rgb(255, 255, 255);">Extruder <?=$codigo_bd[$reg]?><br />
			  ...
			  <td width="126" ><strong>Kilogramos</strong></td>
				...
				<?php for($a=1;$a<=$nEntradas;$a++) { ?>
					...
					<tr>
                          ...
                          <td align="left" ><input class="text" type="text" name="ot_extruder<?=$codigo_bd[$reg]?>[]"/></td>

                          <td align="left"><input class="text2" type="text" name="kg_extruder<?=$codigo_bd[$reg]?>[]" id="<?=$a?>_extr<?=$id_bd[$reg]?>_bd" onkeyup="addTextExtruder(<?=$codigo_bd[$reg]?>,<?=$id_bd[$reg]?>,<?=$a?>,<?=$_REQUEST['turno']?>);" onchange=" sumar_bd('<?=$id_bd[$reg]?>');  sumarExtr_bd(); kh(<?=$_REQUEST['turno']?>);"/></td>
				...
				<?php } ?>
					...
					<tr>
						...
						<td><input style="width:100px" type="text" name="subtotal_extruder<?=$codigo[$reg]?>" id="subtotal_<?=$id_bd[$reg]?>_extr_Total_bd" onChange="sumarExtr_bd();"  readonly="readonly" /></td>
  		...
  		<div id="datosgenerales" style="background-color:#FFFFFF;">
  		  ...
  		  <table  align="center">
                    ...
                    <tr>
                      ...
                      <td style="color:#0000ff;"><input type="text" readonly="readonly" size="20" name="k_h" id="kilo_horas" value=""	 />
                    ...
                    <tr>
                      ...
                      <td class="7"><input type="text" readonly="readonly" size="20" name="k_h_bd" id="kilo_horas_bd" value=""	 />
...

{
        ...
        document.getElementById("subtotal_"+parseInt(x)+"_extr_Total").value = total;
